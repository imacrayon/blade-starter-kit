<x-has-inline-field>
    <input type="radio" {{ $controlAttributes }} value="{{ $value }}" {{ $attributes->class([
      ...
      'before:absolute before:inset-1 before:rounded-full before:bg-white not-checked:before:hidden checked:border-(--color-accent-foreground) checked:bg-(--color-accent-foreground)',
